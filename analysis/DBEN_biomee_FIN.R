...
params_species <- tibble(
  # species         0 1 2 3 4    ...
  lifeform      = c(0,1,1,1,1,rep(1,11)),         # 0 for grasses; 1 for trees
  phenotype     = c(0,1,1,1,1,rep(1,11)),         # 0 for Deciduous; 1 for Evergreen
  ...
  m_cond        = c(7.0,9.0,9.0,9.0,9.0,rep(9.0,11)),                   # factor of stomatal conductance
  ...
  maturalage    = c(0,5,5,5,5,rep(5,11)),        # the age that can reproduce
  fNSNmax       = rep(5,16),                      # multiplier for NSNmax as sum of potential bl and br
  LMA           = c(0.025,0.14,0.14,0.14,0.14,rep(0.14,11)),  # Leaf mass per unit area. In Ensheng rep(0.035,16)
  rho_wood      = c(120,300,300,300,300,rep(300,11)),         # wood density In Ensheng rep(300,16),
  alphaBM       = rep(5200,16),                   
  thetaBM       = rep(2.5,16), 
  # add calibratable params
  kphio         = rep(0.05,16),
...
out_sc1$data[[1]]$output_annual_cohorts %>% filter(PFT==1) %>% group_by(PFT,year) %>%
