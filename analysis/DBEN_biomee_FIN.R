...
params_species <- tibble(
  ...
  LMA           = c(9999,0.02,0.08,0.025,0.14,rep(0.14,11)),  # Leaf mass per unit area. In Ensheng rep(0.035,16)
  rho_wood      = c(9999,120,300,350,300,rep(300,11)),         # wood density In Ensheng rep(300,16),
  alphaBM       = rep(5200,16),                   
  thetaBM       = rep(2.5,16), 
  # add calibratable params
  kphio         = rep(0.05,16),
  phiRL         = rep(3.5,16), #c(9999,1.5,1.2,0.9,1.2,rep(1.2,11)), #rep(3.5,16), ! Root/Leaf area ratio
  LAI_light     = rep(3.5,16),               # Light-limited crown LAI
  v_seed        = c(9999,0.1,0.1,0.1,0.1,rep(0.1,11))        # seed allocation
) 
