...
params_species <- tibble(
  ...
  pt            = c(1,rep(0,15)),                 # 0 for C3; 1 for C4
  # Root parameters
  ...
  LMA           = c(0.025,0.03,0.07,0.07,rep(0.07,12)),  # Leaf mass per unit area. In Ensheng rep(0.035,16)
  rho_wood      = c(90,250,360,360,rep(360,12)),         # wood density In Ensheng rep(300,16),
  alphaBM       = rep(5200,16),                   
  thetaBM       = rep(2.5,16), 
  # add calibratable params
  kphio         = rep(0.05,16),
...
#spinup_forcing <- biomee_forcing_BCI %>% filter(year==2020) %>% rename(yearID=year) 
